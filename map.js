...
const TripMap=(()=>{
 let map,group,filter='all',selected=null;const markers=new Map();
 function selectPlace(id,fromList=false){const marker=markers.get(id);if(!marker)return;selected=id;for(const [key,m] of markers){m.setStyle({weight:key===id?4:2,fillOpacity:key===id?1:.75});m.setRadius(key===id?13:9);m.getElement()?.setAttribute('aria-pressed',String(key===id))}document.querySelectorAll('[data-map-row]').forEach(row=>{const chosen=row.dataset.mapRow===id;row.classList.toggle('map-selected',chosen);row.querySelector('[data-map-select]')?.setAttribute('aria-pressed',String(chosen))});marker.bringToFront();marker.openPopup();const row=document.querySelector('[data-map-row="'+id+'"]');if(fromList){map.setView(marker.getLatLng(),Math.max(map.getZoom(),16));document.querySelector('#trip-map').scrollIntoView({block:'nearest',behavior:'auto'})}else row?.scrollIntoView({block:'nearest',behavior:'auto'});document.querySelector('#map-selection').textContent='נבחר: '+lookup(id).name;}
 ...
 function list(){const visible=PLACES.filter(p=>ids().has(p.id));group.clearLayers();markers.clear();selected=null;document.querySelector('#map-selection').textContent='לחצו על שם מקום או על נקודה כדי לקשר ביניהם.';let bounds=[];
 document.querySelector('#map-list').innerHTML=visible.map(p=>{const day=DAYS.findIndex(d=>d.stops.some(s=>s[3]===p.id)||d.options?.some(o=>o[2]===p.id));const pt=MAP_POINTS[p.id];if(pt){const marker=L.circleMarker([pt.lat,pt.lon],{radius:9,color:colors[Math.max(0,day)],fillOpacity:.9}).addTo(group);const el=document.createElement('div');el.textContent=p.name;marker.bindPopup(el);markers.set(p.id,marker);marker.on('click',()=>selectPlace(p.id));const path=marker.getElement();if(path){path.setAttribute('tabindex','0');path.setAttribute('role','button');path.setAttribute('aria-label',p.name);path.setAttribute('aria-pressed','false');path.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();selectPlace(p.id)}})}bounds.push([pt.lat,pt.lon])}return '<article class="map-place" data-map-row="'+p.id+'"><span class="map-dot" style="background:'+colors[Math.max(0,day)]+'"></span><div>'+(pt?'<button class="map-place-name" data-map-select="'+p.id+'" aria-pressed="false">'+esc(p.name)+'</button>':'<b>'+esc(p.name)+'</b>')+'<p>'+esc(p.area)+(pt?'':' · מיקום על המפה ממתין לאימות')+'</p><div class="actions">'+external(pin(p),'חיפוש במפה')+external(waze(p.map),'Waze')+'<button class="softbutton" data-place="'+p.id+'">פרטים</button></div></div></article>'}).join('');
 document.querySelector('#map-count').textContent=visible.length+' מקומות · '+bounds.length+' נקודות על המפה';if(bounds.length)map.fitBounds(bounds,{padding:[35,35],maxZoom:16});
 }
 function show(){if(map){map.remove();map=null}main.innerHTML='<section class="map-title"><p class="eyebrow">כל ההרפתקאות על מפה אחת</p><h2>מה קרוב למה?</h2><p>בחרו יום כדי לצמצם את רשימת המקומות. המיקומים נוספו מתוצאות חיפוש ואינם בהכרח הכניסה המדויקת. מקומות ללא נקודה זמינים בקישור לחיפוש.</p></section><div class="map-toolbar"><label>יום בטיול <select id="map-day"><option value="all">כל הימים</option>'+DAYS.map((d,i)=>'<option value="'+i+'" '+(filter===String(i)?'selected':'')+'>'+esc(d.date+'.08 · '+d.short)+'</option>').join('')+'</select></label><span id="map-count"></span><button class="softbutton" id="map-background">טעינת מפת רקע · דורש אינטרנט</button></div><p id="map-selection" class="map-selection" role="status"></p><div class="map-layout"><div id="trip-map" aria-label="מפת מקומות הטיול"></div><div id="map-list"></div></div><p class="source">נתוני מיקום: © OpenStreetMap contributors. רשימת המקומות זמינה אופליין; מפת הרקע דורשת אינטרנט. המפה אינה מסלול נהיגה ואינה מאמתת זמני נסיעה.</p>';
 ...
 document.querySelector('#map-list').addEventListener('click',e=>{const button=e.target.closest('[data-map-select]');if(button)selectPlace(button.dataset.mapSelect,true)});
